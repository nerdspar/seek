...
| An episode's play count is `consumptions_number`, not `progress` | On the episode detail endpoint `progress` is always null; the number of plays is `consumptions_number` (and `len(consumptions)`). Reading `progress` to count plays silently yields 0 every time — it defeated the rewatch guard on the un-complete-a-show walk-back until caught. Show/season rows *do* use `progress` for episodes-watched; only the single-episode detail differs. |
| `next_episode` is the LOWEST unwatched episode, not next-up | Only the same thing when what you have watched is an unbroken run from the start. Below Deck Mediterranean, 31 episodes in across seasons 10–11, reported `next_episode: S01E01` — so the row sat at the top of "recently watched" describing an episode from 2016. Seek corrects rows where `progress > 0` and next is S01E01, which cannot both be true, by reading the most recently progressed season and taking the episode after the last one played. |
